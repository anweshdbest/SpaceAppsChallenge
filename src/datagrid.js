...
define(function() {
    ...
    var createGrid = function(ISS_Data) {
        var pqSearch = {
            ...
            results : null,
            prevResult : function() {
                var colIndx = this.colIndx, rowIndices = this.rowIndices;
            ...
            updateSelection : function() {
                var colIndx = this.colIndx, curIndx = this.curIndx, rowIndices = this.rowIndices;
            ...
            search : function() {
                var txt = $("input.pq-search-txt").val().toUpperCase(), colIndx = $("select#pq-crud-select-column").val(), DM = $grid.pqGrid("option", "dataModel"), sortIndx = DM.sortIndx, sortDir = DM.sortDir;
            ...
            render : function(ui) {
                rowIndx = ui.rowIndx, rowData = ui.rowData, dataIndx = ui.dataIndx, colIndx = ui.colIndx, val = rowData[dataIndx];

                if (ui.customData) {
                    var rowIndices = ui.customData.foundRowIndices, searchColIndx = ui.customData.searchColIndx, txt = ui.customData.txt, txtUpper = txt.toUpperCase(), valUpper = val.toUpperCase();
        ...
        $.extend(newObj.colModel[1], {
            render : function(ui) {
                rowData = ui.rowData, dataIndx = ui.dataIndx, colIndx = ui.colIndx, value = rowData[dataIndx];
                if (value instanceof JulianDate) {
                    ui.rowData[ui.dataIndx] = value.toIso8601();
                }
        ...
        $("#grid_search").on("pqgridrender", function(evt, obj) {
            ...
            $("<button title='Previous Result'></button>").appendTo($toolbar).button({
                icons : {
            ...
            $("<button title='Next Result'></button>").appendTo($toolbar).button({
                icons : {
